...
function render() {
    ...
    for (let i = 0; i < state.tasks.length; i++) {
        myNodeList.insertAdjacentHTML('beforeend',
            `<li class="${state.tasks[i].checked ? `checked` : ''} ${state.tasks[i].complete ? `complete` : ''} ${state.tasks[i].outcome ? `outcome` : ''} ${state.tasks[i].redaction ? `redaction` : ''} task" data-id="${state.tasks[i].id}" draggable="${state.tasks[i].draggable}"><button class="after-check" id="done">Выполнено</button><button class="after-check" id="loading" >В процессе</button><button class="after-check" id="undone"  > Не выполнено</button><div><p> ${state.tasks[i].redaction ? `<input type="text" value="${state.tasks[i].name}" id="update-input" class="redaction"/>` : `<span>${state.tasks[i].name} </span>`}${(state.tasks[i].complete || state.tasks[i].outcome) ? '' : ` <small id="${state.tasks[i].redaction ? 'save' : 'change'}" >${state.tasks[i].redaction ? 'Сохранить' : 'Редактировать'}</small> ${!state.tasks[i].redaction ? '' : `<small id="back">Назад</small>`}`}</p>${state.tasks[i].redaction ? `<input type="date" value="${new Date(state.tasks[i].endDate).getFullYear()}-${new Date(state.tasks[i].endDate).getMonth() + 1 < 10 ? `0${new Date(state.tasks[i].endDate).getMonth() + 1}` : new Date(state.tasks[i].endDate).getMonth() + 1}-${new Date(state.tasks[i].endDate).getDate() < 10 ? `0${new Date(state.tasks[i].endDate).getDate()}` : new Date(state.tasks[i].endDate).getDate()}" id="update-date" />` : `<div class="date"> <span>Start date: <b class="start-date">${new Date(state.tasks[i].startDate).toLocaleDateString()}</b></span><span>End date <b class="end-date">${new Date(state.tasks[i].endDate).toLocaleDateString()}</b></span></div>`}</div>${state.tasks[i].closable ? `  <span class="close" id="close">\u00D7</span>` : ''}</li>`)
    }
}
...
function completeTask(id) {
    let [task, idx] = getTask(id);
    state.tasks[idx] = {...task, complete: true, checked: false, outcome: false}
    localStorage.setItem('tasks', JSON.stringify(state.tasks));
    render()
...
function outcomeTask(id) {
    let [task, idx] = getTask(id);
    state.tasks[idx] = {...task, outcome: true, checked: false, complete: false}
    localStorage.setItem('tasks', JSON.stringify(state.tasks));
    render()
}
...
function loadingTask(id) {
    let [task, idx] = getTask(id);
    state.tasks[idx] = {...task, checked: false, complete: false, outcome: false}
    localStorage.setItem('tasks', JSON.stringify(state.tasks));
    render()
}
...
function newElement() {
    ...
    if (inputValue === '' || new Date(endDate) < new Date()) {
...
tasksListElement.addEventListener(`dragover`, (evt) => {
    ...
    //
    //
    // const chosenTask = state.tasks.findIndex(t => t.id === +activeElement.dataset.id)
    // let changeTask = state.tasks.findIndex(t => {
    //     if (nextElement === null){
    //         return state.tasks.length
    //     }
    //     return (t.id === +nextElement.dataset.id)
    // });


    // if (changeTask === 0){
    //     changeTask = state.tasks.length
    // }

    //console.log(chosenTask, changeTask)
    //
    // let tmp = state.tasks[changeTask-1]
    // state.tasks[changeTask-1] = {...state.tasks[chosenTask]}
    // state.tasks[chosenTask] = {...tmp}
